...
void Config::SetCfgFile(const std::string& cfg_file) throw(Exception)
{
	if ( cfg_file.empty() )
	{
		throw Exception(ERR_CFG_INVALID_FILE, "The configuration file_path is empty! [FILE:%s, LINE:%d]", __FILE__, __LINE__);
	}

	if ( !IsRegularFile(cfg_file) )
	{
		throw Exception(ERR_CFG_INVALID_FILE, "The configuration file \"%s\" is not a regular file! [FILE:%s, LINE:%d]", cfg_file.c_str(), __FILE__, __LINE__);
	}
...
void Config::ReadConfig() throw(Exception)
{
	if ( m_cfgFile.empty() )
	{
		throw Exception(ERR_CFG_INVALID_FILE, "The configuration file unset! [FILE:%s, LINE:%d]", __FILE__, __LINE__);
	}

	BaseFile m_bfCfg;
	if ( !m_bfCfg.Open(m_cfgFile) )
	{
		throw Exception(ERR_CFG_OPEN_FILE_FAIL, "Open configuration file \"%s\" fail! [FILE:%s, LINE:%d]", m_cfgFile.c_str(), __FILE__, __LINE__);
	}
...
std::string Config::GetCfgValue(const std::string& segment, const std::string& name) throw(Exception)
{
	...
	if ( FindItem(str_seg, str_name, &it) )
	{
		if ( !(it->m_bFind) )
		{
			throw Exception(ERR_CFG_ITEM_NOT_FOUND, "Configure item [%s->%s] not found! [FILE:%s, LINE:%d]", str_seg.c_str(), str_name.c_str(), __FILE__, __LINE__);
		}
		else if ( it->m_value.empty() )
		{
			throw Exception(ERR_CFG_INVALID_VALUE, "Configure item [%s->%s] value is invalid! [FILE:%s, LINE:%d]", str_seg.c_str(), str_name.c_str(), __FILE__, __LINE__);
		}
	...
	{
		throw Exception(ERR_CFG_UNREGISTER_ITEM, "Configure item [%s->%s] unregistered! [FILE:%s, LINE:%d]", str_seg.c_str(), str_name.c_str(), __FILE__, __LINE__);
		return std::string();
	}
}
...
double Config::GetCfgDoubleVal(const std::string& segment, const std::string& name) throw(Exception)
{
	...
	{
		throw Exception(ERR_CFG_INVALID_VALUE, "Configure item [%s->%s] value ('%s') convert to double type failed! [FILE:%s, LINE:%d]", segment.c_str(), name.c_str(), d_str.c_str(), __FILE__, __LINE__);
	}
...
long long Config::GetCfgLongVal(const std::string& segment, const std::string& name) throw(Exception)
{
	...
	{
		throw Exception(ERR_CFG_INVALID_VALUE, "Configure item [%s->%s] value ('%s') convert to long long type failed! [FILE:%s, LINE:%d]", segment.c_str(), name.c_str(), ll_str.c_str(), __FILE__, __LINE__);
	}
...
unsigned int Config::GetCfgUIntVal(const std::string& segment, const std::string& name) throw(Exception)
{
	...
	{
		throw Exception(ERR_CFG_INVALID_VALUE, "Configure item [%s->%s] value ('%s') convert to unsigned int type failed! [FILE:%s, LINE:%d]", segment.c_str(), name.c_str(), u_str.c_str(), __FILE__, __LINE__);
	}
