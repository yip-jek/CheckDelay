...
std::string BaseDir::DescribeFileType(BaseDir::DirFileType type)
{
	switch ( type )
	{
	case DFT_DIR: return "DIR";
	case DFT_REG: return "REGULAR";
	case DFT_LNK: return "LINK";
...
std::string BaseDir::GetPath() const
{
	return m_strPath;
}
...
bool BaseDir::GetFileName(std::string& file_name, DirFileType& type, long long& size, time_t& time)
{
	struct dirent* p_dnt = NULL;
	struct stat st;
	std::string str_fullname;
	DirFileType df_type = DFT_Unknown;
	...
	while ( (p_dnt = readdir(m_pDir)) != NULL )
	{
		...
		if ( lstat(str_fullname.c_str(), &st) != 0 )
		{
			continue;
		}
...
bool BaseDir::GetFullName(std::string& full_name, DirFileType& type, long long& size, time_t& time)
{
	if ( GetFileName(full_name, type, size, time) )
	{
